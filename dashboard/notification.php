		...
		<div id="content">
									...
									<ul class="nav navbar-nav flex-row ml-auto">
										<li class="dropdown nav-item active">
											...
											<ul class="dropdown-menu">
												...
												<li><a href="#"><?php echo '<h5>'.$row['title'].'</h5>'.'<br>'.$row['text'] ;?></a></li><hr>
												<?php }?>
											</ul>
			...
			<div class="main-content">
				<div class="row">
					<div class="col-md-12">
						<div class="table-wrapper">
							...
							<div class="table-title">
								<div class="row">
									<div class="col-sm-6 p-0 flex justify-content-lg-start justify-content-center">
										<h2 class="ml-lg-2">Manage notification</h2>
									</div>
							...
							<table class="table table-striped table-hover">
								...
								<tbody>
									<?php $query = mysqli_query($con, "select * from notification_list limit $page1,5 ");
																						$num=mysqli_num_rows($query);
									...
									?>
										<tr>
											...
											<th class="review"><?php echo $row['text']; ?></th>
						...
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								...
								<div class="modal-body">
									<form action="notification_crud.php" method="post" >
									...
									<div class="form-group">
										...
										<select name="users" id="">
											...
											<?php $query=mysqli_query($con,"select * from role") ;
											while ($row=mysqli_fetch_assoc($query)) {
											?>
								...
								<div class="modal-footer">
									...
									<button type="submit" name="add_notification" class="btn btn-success">Add</button>
								</div>
							</div>
							</form>
						</div>
					...
					$query=mysqli_query($con,"call get_notification($id);");
					while ($row=mysqli_fetch_assoc($query)) {
					...
					<div class="modal fade" tabindex="-1" id="edit" role="dialog">
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								...
								<div class="modal-body">
									<form action="notification_crud.php?id=<?php echo $id; ?>" method="post" >
									<div class="form-group">
										<label>title</label>
									...
									<div class="form-group">
										...
										<select name="users" id="">
											...
											<?php
                                            require '../connection.php';
                                            $stmt = mysqli_query($con, "select * from role;");
                                            if (mysqli_num_rows($stmt) > 0) {
                                                while ($cat = mysqli_fetch_assoc($stmt)) { ?>
                                                    <option <?php if ($cat['role_id'] == $row['users']) {
                                                                print ' selected ';
                                                            } ?> value="<?php echo $cat['role_id']; ?>"><?php echo $cat['role_name']; ?></option>
									...
									<div class="form-group">
										<label>text</label>
										<textarea class="form-control"  name="text" required><?php echo $row['text'] ?></textarea>
									</div>
									
								</div>
								<div class="modal-footer">
									<button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
									<button type="submit" name="update_notification" class="btn btn-success">update</button>
								</div>
								</form>
							</div>
						</div>
						...
						<div class="modal fade" tabindex="-1" id="delete" role="dialog">
							<div class="modal-dialog" role="document">
								<div class="modal-content">
									...
									<div class="modal-footer">
										<button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
										<a href="notification_crud.php?no=<?php echo $id; ?>" class="btn btn-success">delete</a>
									</div>
								</div>
							</div>
						</div>
